...
import { OpportunitiesResponse } from '@/app/types/recruiter'
const PAGE_SIZE = 12
...
export function OpportunitiesComponent({sports}: {sports: Sport[]}) {
      ...
      <section className="bg-white border sticky top-16 z-40 rounded-2xl px-3">
        <div className="container py-6">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {[{ name: 'All Sports', id: 'all' }, ...sports].map((filter) => (
              <Button
                key={filter.id}
                variant={filter.name === selectedSport ? 'default' : 'outline'}
                size="sm"
                className="rounded-full"
                onClick={() =>
                  updateParams({ sport: filter.name === 'All Sports' ? null : filter.name })
                }
              >
                {filter.name}
              </Button>
