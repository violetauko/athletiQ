    ...
    <div className="flex flex-col">
          ...
          <div className="flex flex-col md:flex-row gap-2 md:gap-4">
            <div className="flex-1 flex gap-2">
              ...
              <Button className="bg-black hover:bg-black/90" onClick={handleSearch}>
                <Search className="w-3 md:w-4 h-3 md:h-4 mr-2 text-sm md:text-base" />
                Search
              </Button>
            </div>
            <Button variant="outline" className="gap-2">
              <Filter className="w-3 md:w-4 h-3 md:h-4 text-sm md:text-base" />
              Filters
            </Button>
          </div>
      ...
      <section className="py-6 md:py-12 flex-1">
        <div className="container">
          <div className="flex items-center justify-between mb-4 md:mb-8">
            <div className="text-muted-foreground">
              {isLoading ? (
                ...
                <>
                  Showing{' '}
                  <span className="font-semibold text-foreground text-sm md:text-base">{data?.pagination.total ?? 0}</span>{' '}
                  opportunities
                </>
              )}
            </div>
          ...
          {isError && (
            <div className="text-center py-8 md:py-16 text-muted-foreground text-sm md:text-base">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
              {Array.from({ length: PAGE_SIZE }).map((_, i) => (
                <Skeleton key={i} className="h-48 md:h-64 rounded-2xl" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
...
export default function OpportunitiesPageWrapper() {
  return (
    <Suspense fallback={<div className="text-center py-12 md:py-20">Loading…</div>}>
      <OpportunitiesPage />
    </Suspense>
  );
}